    ...
    <div class="row col-12">
      ...
      <div class="container text-center mb-4">
        <div class="row row-cols-auto">
          <div class="col">
            <router-link :to="{ path: '/assignmenu' }"> Menú de asigación </router-link>
          </div>
          >
          <div class="col text-primary">
            <a class="text-dark" href="#">Estudiantes con patrocinadores</a>
          </div>
        </div>
      </div>
      <!--Exportación-->
...
const headers = [
  ...
  { title: "Cantidad de patrocinadores", key: "cantidadPatrocinadores" },
...
const getEstudiantes = async () => {
  try {
    const response = await axios.get(`/api/EstudiantePatrocinador/listaEstudiantes`);
    estudiantes.value = response.data.filter(
      (estudiante) => estudiante.estatus.trim().toUpperCase() === "A"
    );

    mapeoDeDato(estudiantes.value);
  } catch (error) {
    Swal.fire({
      title: "Error",
...
const mapeoDeDato = (datos) => {
  resultadoFiltrado.value = datos.map((estudiante, index) => ({
    ...
    cantidadPatrocinadores: estudiante.cantidadPatrocinadores,
    estado: estudiante.estatus,
  }));
};
...
const buscarEstudiante = () => {
  ...
  if (textoBusqueda !== "") {
    datosFiltrados = estudiantes.value.filter((estudiante) => {
...
//#region Exportar datos de patrocinadores asignados
...
const descargarPDF = async (datos) => {
  ...
  doc.setFontSize(12);
  doc.text("ESTUDIANTES - PATROCINADORES", 70, 7);
  doc.text("ASOCIACIÓN QUCHOOCH", 79, 14);

  // Encabezados de la tabla
  const headers2 = ["No.", "Código", "Nombre", "Apellido", "Cantidad de patrocinadores"];
  ...
  const data = datos.map((estudiante, index) => [
    ...
    estudiante.cantidadPatrocinadores,
  ]);
...
const exportarExcel = () => {
  ...
  const data = datos.map((estudiante, index) => ({
    ...
    "Cantidad de patrocinadores": estudiante.cantidadPatrocinadores,
  }));
