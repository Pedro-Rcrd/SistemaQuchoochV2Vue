            ...
            <div class="card border pt-3" id="exportarElemento">
                ...
                <div class="card-body ">
                    <div class="row">
                        ...
                        <div class="bg-light p-2">
                            ...
                            <div class="row">
                                <div class="col-7">
                                    <p class="m-0">
                                        Nombre: <strong>Pedro Ricardo Lopez</strong>
                                    </p>
                                    <p class="m-0">
                                        Código: <strong>AQ012</strong>
                                    </p>
                    ...
                    <div class="row justify-content-center text-center mt-3">
                        <h5><i @click="exportarExcel" v-show="ocultarElementos" style="font-size: 17px;"
                                ...
                                <table class="table table-scroll table-sm table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" class="text-center" v-show="ocultarElementos">Acción</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(patrocinador, index) in patrocinadoresAsignados" :key="index">
                                            ...
                                            <td class="text-center" v-show="ocultarElementos"
                                            @click="confirmDelete(patrocinador.nombrePatrocinador, patrocinador.codigoEstudiantePatrocinador)"><i
                    ...
                    <div class="" v-show="ocultarElementos">
                            ...
                            <div class="col-md-8">
                                ...
                                <div class="input-group mb-3">
                                    ...
                                    <input class="form-control" autofocus id="codigoEstudiante" required
                                        v-model="busqueda" @input="buscarPatrocinador" type="text"
                            ...
                            <div class="col-md-8 mb-1  justify-content-center">
                                <div class="table-container">
                                    <table class="table table-scroll">
                                        ...
                                        <tbody>
                                            <tr v-for="(patrocinador, index) in listaNuevosPatrocinadores" :key="index">
                                                <th scope="row" class="text-center">{{ index + 1 }}</th>
                                                <td class="text-start">{{ patrocinador.nombrePatrocinador }}</td>
                                                <td class="text-center">
                                                    <i @click.prevent="eliminarPatrocinador(index)"
                        ...
                        <RouterLink :to="{ name: 'studentssponsors' }" type="button" class="btn btn-outline-primary">
                            Cancelar
                        </RouterLink>
                        <button type="submit" @click.prevent="save" class="btn btn-primary text-light"
                            :disabled="deshabilitarComponentes"><i class="fa-solid fa-save"></i> Guardar
                            cambios</button>
                    </div>

                    </div> <!--sección agregar nuevos patrocinadores-->
...
const getPatrocinadoresAsignados = async () => {
    try {
        const response = await axios.get(`/api/estudiantePatrocinador/listaPatrocinadores/${parametro}`);
...
const getPatrocinadores = async () => {
    try {
        ...
        patrocinadores.value = response.data.filter(a => a.estado.trim().toUpperCase() === "A");
...
const buscarPatrocinador = () => {
    ...
    if (textoBusqueda !== '') {
        ...
        visibilidad.value = patrocinadores.value.filter(patrocinador =>
            (patrocinador.nombrePatrocinador.toLowerCase() + " " + patrocinador.apellidoPatrocinador.toLowerCase())
...
const seleccionarPatrocinador = (patrocinador) => {
    ...
    busqueda.value = patrocinador.nombrePatrocinador + " " + patrocinador.apellidoPatrocinador;
...
const agregarPatrocinador = () => {
    if (codigoPatrocinadorSeleccionado.value !== 0 && codigoPatrocinadorSeleccionado.value !== "") {
        const codigoPatrocinadorExistente = listaNuevosPatrocinadores.value.find(a => a.codigoPatrocinador === codigoPatrocinadorSeleccionado.value);
...
const save = async () => {
    try {
        if (listaNuevosPatrocinadores.value !== null, listaNuevosPatrocinadores.value != "") {

            deshabilitarComponentes.value = true;
            const response = await axios.post('/api/estudiantePatrocinador/create', listaNuevosPatrocinadores.value);
