<template>
  <div class="row justify-content-center mt-1" v-if="authStore.moduloAsignacion">
        ...
        <div class="card-body">
          ...
          <div class="inf" id="exportarElemento">
            <div class="card border mt-3">
              <h5>
                <strong>Nombre del estudiante: </strong> {{ estudiante }}
                {{ apellidoEstudiante }}
              </h5>
                ...
                <div class="table-responsive" v-else>
                  <table class="table table-bordered table-hover">
                    ...
                    <tbody class="table-group-divider">
                      <tr
                        v-for="(lp, i) in listaPatrocinadores"
                        :key="lp.codigoEstudiantePatrocinador"
                      >
                        <td>{{ i + 1 }}</td>
...
const deletePatrocinador = (id, name) => {
  confirmation(
    name,
    `/api/estudiantepatrocinador/delete/${id}`,
    `/assignsponsor/${parametro}`,
    authStore.authToken
  );
};
