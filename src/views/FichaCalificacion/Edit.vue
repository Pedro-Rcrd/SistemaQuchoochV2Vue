                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                ...
                                <span><i data-bs-toggle="modal" data-bs-target="#modalEstablecimiento"
                                        ...
                                        @click="noMostrarAlertas(), openModal(2)"></i></span>
                                <div class="input-group mb-3">
                                    <span class="input-group-text">
                                        ...
                                        <table class="table table-striped table-sm">
                                            ...
                                            <tbody>
                                                <tr v-for="(materia, index) in item.materias" :key="index">
                                                    ...
                                                    <td>{{ materia.nota }}</td>
                                                    <td> <!-- Puedes agregar una acción aquí si es necesario -->
            ...
            <div class="row col-11">
                <form @submit.prevent="save(4)">
                    <div class="input-group mb-3">
                        ...
                        <input autofocus type="number" v-model="nuevaNotaCalificacion" min="1" max="100"
                            placeholder="Nota" required class="form-control" ref="nameInput">
                    </div>
                    <div class="d-grid col-6 mx-auto">
                        <button class="btn btn-dark">
                            <i class="fa-solid fa-save"></i> Registrar</button>
...
const save = async (opcion) => {
    if (opcion == 1) {
        ...
        try {
            mostrarLoading.value = true;
            const response = await axios.put(`/api/fichaCalificacion/actualizarNotaCurso/${codigoCursoFichaCalificacion.value}?notaCalificacion=${nuevaNotaCalificacion.value}`);
            mostrarLoading.value = false;
            if (response.data.estatus = true) {
                mostrarAlertaSuccess.value = true;
