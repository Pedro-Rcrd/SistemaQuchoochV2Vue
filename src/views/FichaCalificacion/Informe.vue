      ...
      <div class="row">
        <div class="col-10">
          <div class="row">
            <div class="col-2">
              <input
                @change="ConsultarFichasNuevamente"
          ...
          <div class="row mt-1">
            ...
            <div class="col-md-4">
              ...
              <div class="input-group">
                <button @click="getFichasPorRangoFecha" class="btn btn-primary">
      ...
      <div class="row justify-content-between">
        <div class="col-8">
          <input
            ...
            v-model="filtro"
            @input="buscarfichasCalificaciones"
            type="text"
        ...
        <div class="col-2">
          <input
            v-model="mostrarTodasLasfichasCalificaciones"
            type="checkbox"
            class="form-check-input"
            id="checkRegistro"
          />
      ...
      <v-card>
        <v-data-table density="compact" :items="resultadoFiltrado" :headers="headers">
          <template v-slot:item.actions="{ item }">
            <router-link
              :to="{ path: '/informationcard/' + item.codigoFichaCalificacion }"
              ><v-icon class="mr-2" size="small">
                <i class="fa-solid fa-eye text-dark"></i>
              </v-icon>
            </router-link>
            <router-link
              :to="{ path: '/exportarFichaEscolar/' + item.codigoFichaCalificacion }"
              ><v-icon size="small">
...
const getFichasCalificaciones = async () => {
  try {
    ...
    fichasCalificacionesActivas.value = fichasCalificaciones.value.filter(
      (ficha) => ficha.estatus.trim().toUpperCase() === "A"
    );
...
const getFichasPorRangoFecha = async () => {
  try {
    if (!formRangoFecha.value.fechaIncio || !formRangoFecha.value.fechaFin) {
      return Swal.fire({
        title: "¡Campos vacíos!",
        text: `Por favor, complete el rango de fechas.`,
        icon: "warning",
      });
    }
