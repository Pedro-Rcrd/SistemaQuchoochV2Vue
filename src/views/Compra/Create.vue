<template>
  <div class="row justify-content-center mt-1" v-if="authStore.moduloCompra">
...
const getProveedores = async () => {
  try {
    ...
    proveedores.value = response.data.filter(
...
const submitForm = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      (productos.value = []), (personaRecibe.value = "");
      total.value = "";
      descripcion.value = "";
