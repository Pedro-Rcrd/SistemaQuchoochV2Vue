                ...
                <div class="card-body">
                    <form enctype="multipart/form-data" @submit.prevent="submitForm">
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            ...
                            <button type="submit" class="btn btn-primary text-light" :disabled="botonDeshabilitado"><i
...
const getEstudiante = async () => {
    try {
        ...
        codigoCarrera.value = response.data.codigoCarrera !== null ? response.data.codigoCarrera : "";
        ...
        Swal.fire({
            title: 'Error',
            text: `Hubo un error al intentar obtener la información del estudiante`,
            icon: 'error',
...
const submitForm = async () => {
    if (!codigoBecario.value || !nombreEstudiante.value || !apellidoEstudiante.value ||
        ...
        !codigoEstablecimiento.value || !nombrePadre.value || !nombreMadre.value || !oficioPadre.value || !oficioMadre.value || !fechaRegistro.value) {
        return Swal.fire({
            icon: 'warning',
    ...
    try {
        ...
        const response = await fetch(`${baseApiBackend}/api/estudiante/update/${parametro}`, {
            method: 'PUT',
            body: formData,
            headers,
        });
        if (!response.ok) {
            const errorResponse = await response.json(); // Para obtener el contenido del error del backend
        ...
        if (response.ok) {
            Swal.fire({
                icon: 'success',
                title: "La información del estudiante ha sido modificado exitosamente",
                showConfirmButton: false, // Ocultar el botón "Aceptar"
            ...
            Swal.fire({
                icon: 'error',
                title: "Hubo un error al modificar la información del estudiante",
                showConfirmButton: false, // Ocultar el botón "Aceptar"
                timer: 1500, // Tiempo en milisegundos antes de que se cierre automáticamente
            });
        }
