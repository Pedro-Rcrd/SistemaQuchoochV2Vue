  ...
  <div class="row justify-content-center">
    <div class="row col-12">
      ...
      <div class="row justify-content-between">
        ...
        <div class="col-2">
          <input
            v-model="mostrarTodosLosProveedores"
            type="checkbox"
        ...
        <v-data-table density="compact" :items="resultadoFiltrado" :headers="headers">
          <template v-slot:item.actions="{ item }">
            ...
            <v-icon
              size="small"
              @click="
                ($event) => confirmDelete(item.nombreProveedor, item.codigoProveedor)
              "
            >
...
const getProveedores = async () => {
  try {
    ...
    proveedoresActivos.value = proveedores.value.filter(
      (proveedor) => proveedor.estatus.trim().toUpperCase() === "A"
    );
...
const save = async () => {
  ...
  if (operation.value == 1) {
    ...
    res = await sendRequest(
      "PUT",
      form.value,
      `/api/proveedor/update/${codigoProveedorActualizar.value}`,
      ""
    );
...
const descargarPDF = async (datos) => {
  ...
  doc.setFontSize(12);
  doc.text("LISTADO DE PROVEEDORES", 77, 7);
  doc.text("ASOCIACIÓN QUCHOOCH", 79, 14);

  // Encabezados de la tabla
  const headers2 = ["No.", "Nombre", "Encargado", "Teléfono", "Descripción", "Estado"];
