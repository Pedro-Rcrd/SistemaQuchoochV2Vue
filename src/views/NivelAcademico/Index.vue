<template>
  <div class="row justify-content-center">
    <div class="row col-12">
      <h2>Lista de niveles academicos</h2>
      <hr />
      <!--Menún de navegación-->
      <div class="container text-center mb-4">
        <div class="row row-cols-auto">
          ...
          >
          <div class="col">
            <a class="text-dark" href="#">Niveles académicos</a>
          </div>
        </div>
      </div>
      ...
      <div class="row justify-content-between">
        ...
        <div class="col-2">
          ...
          >
            ...
            <option value="nombreNivelAcademico">Nombre del nivel academico</option>
            <option value="estatus">Estado</option>
          </select>
        </div>
        <div class="col-2">
          <input
            v-model="mostrarTodosLosNivelesAcademicos"
            type="checkbox"
        ...
        <v-data-table density="compact" :items="resultadoFiltrado" :headers="headers">
          <template v-slot:item.actions="{ item }">
            ...
            <v-icon
              ...
              @click="
                ($event) =>
                  confirmDelete(item.nombreNivelAcademico, item.codigoNivelAcademico)
              "
            >
        ...
        <form @submit.prevent="save">
          <div class="input-group mb-3">
            ...
            <input
              autofocus
              type="text"
              v-model="form.nombreNivelAcademico"
...
const headers = [
  { title: "#", key: "indice" },
  { title: "Nombre", key: "nombreNivelAcademico" },
  { title: "Estado", key: "estatus" },
  { title: "Acción", key: "actions", sortable: false },
];

//GET nivelesAcademicos
//#region GET nivelesAcademicos
const nivelesAcademicos = ref([]);
const nivelesAcademicosActivos = ref([]);
const nivelesAcademicosSeleccionados = ref([]);
const getNivelesAcademicos = async () => {
  try {
    ...
    // Filtra los nivelesAcademicos cuyo estatus es igual a "A", ignorando mayúsculas/minúsculas
    nivelesAcademicosActivos.value = nivelesAcademicos.value.filter(
      (nivelAcademico) => nivelAcademico.estatus.trim().toUpperCase() === "A"
    );

    if (mostrarTodosLosNivelesAcademicos.value) {
      nivelesAcademicosSeleccionados.value = nivelesAcademicos.value;
    } else {
      nivelesAcademicosSeleccionados.value = nivelesAcademicosActivos.value;
    }

    mapeoDeDato(nivelesAcademicosSeleccionados.value);
  } catch (error) {
    Swal.fire({
      title: "Error",
      text: `Hubo un error al intentar obtener la lista de niveles Academicos.`,
      icon: "error",
...
const mapeoDeDato = (datos) => {
  resultadoFiltrado.value = datos.map((nivelAcademico, index) => ({
    indice: index + 1,
    codigoNivelAcademico: nivelAcademico.codigoNivelAcademico,
    nombreNivelAcademico: nivelAcademico.nombreNivelAcademico,
    estatus: nivelAcademico.estatus,
  }));
};
...
const tipoBusqueda = ref("Buscar nivel academico");
const filtro = ref("");
const resultadoFiltrado = ref([]);
const tipoFiltro = ref("nombreNivelAcademico");
const mostrarTodosLosNivelesAcademicos = ref(false);

//Metodo del filtro de datos en la tabla
const buscarNivelAcademico = () => {
  const textoBusqueda = filtro.value.toLowerCase().trim();
  let datosFiltrados; // Por defecto, usa todos los nivelesAcademicos

  if (textoBusqueda !== "") {
    datosFiltrados = nivelesAcademicosSeleccionados.value.filter((nivelAcademico) => {
      const campoFiltro = nivelAcademico[tipoFiltro.value]?.toLowerCase(); // Utiliza la propiedad basada en tipoFiltro
...
watch(mostrarTodosLosNivelesAcademicos, (newValue) => {
  if (!newValue) {
...
const codigoNivelAcademicoActualizar = ref("");
const close = ref([]);
...
const confirmDelete = (nombre, codigo) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      deletenivelAcademico(nombre, codigo);
    }
  });
};

// Método de eliminación (este es el que ya tienes implementado)
const deletenivelAcademico = async (nombreNivelAcademico, codigoNivelAcademico) => {
  try {
    const response = await axios.delete(
      `/api/nivelAcademico/delete/${codigoNivelAcademico}`
    );
    Swal.fire({
      title: "Eliminado!",
      text: `El nivel académico ${nombreNivelAcademico} ha sido eliminado.`,
      icon: "success",
    });
    getNivelesAcademicos();
  } catch (error) {
    Swal.fire({
      title: "Error",
      text: `Hubo un error al intentar eliminar el nivel académico ${nombreNivelAcademico}.`,
      icon: "error",
...
const openModal = (op, codigoNivelAcademico, nombreNivelAcademico, estatus) => {
  clear();
  setTimeout(() => nextTick(() => nameInput.value.focus()), 600);
  operation.value = op;
  codigoNivelAcademicoActualizar.value = codigoNivelAcademico;
  if (op == 1) {
...
const save = async () => {
  ...
  if (operation.value == 1) {
    ...
    res = await sendRequest(
      "PUT",
      form.value,
      `/api/nivelAcademico/update/${codigoNivelAcademicoActualizar.value}`,
      ""
    );
...
const descargarPDF = async (datos) => {
  ...
  doc.setFontSize(12);
  doc.text("LISTADO DE NIVELES ACADÉMICOS", 69, 7);
  ...
  // Datos de la tabla
  const data = datos.map((nivelAcademico, index) => [
    index + 1,
    nivelAcademico.nombreNivelAcademico,
    nivelAcademico.estatus,
  ]);
  ...
  doc.save("ReporteNivelesAcademicos.pdf");
};
//#endregion
...
const exportarExcel = () => {
  ...
  const data = datos.map((nivelAcademico, index) => ({
    Indice: index + 1,
    Nombre: nivelAcademico.nombreNivelAcademico,
    Estado: nivelAcademico.estatus,
  }));
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Lista de niveles academicos");
  ...
  link.href = url;
  link.download = "reporteNivelesAcademicos.xlsx";
