...
const getComunidades = async (page) => {
    try {
...
const deleteComunidad = (id, name) => {
    confirmation(name, `/api/Comunidad/delete/${id}`, '/communities', authStore.authToken);
};

const openModal = (op, codigoComunidad, nombreComunidad) => {
    clear();
    setTimeout(() => nextTick(() => nameInput.value.focus()), 600);
    operation.value = op;
    id.value = codigoComunidad;
    if (op == 1) {
        title.value = 'Crear '
...
const save = async () => {
    ...
    if (operation.value == 1) {
        ...
        res = await sendRequest('PUT', form.value, `/api/Comunidad/update/${id.value}`, '');
                ...
                <div class="table-responsive" v-else>
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody class="table-group-divider">
                            <tr v-for="c, i in comunidades" :key="c.codigoComunidad">
                                <td>{{ (i + 1) }}</td>

                                <td>{{ c.nombreComunidad }}</td>
                                <td class="text-center">
                                    <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#modal"
                                        @click="$event => openModal(2, c.codigoComunidad, c.nombreComunidad)">
                                ...
                                <td class="text-center">
                                    <button class="btn btn-danger btn-sm"
                                        @click="$event => deleteComunidad(c.codigoComunidad, c.nombreComunidad)">
