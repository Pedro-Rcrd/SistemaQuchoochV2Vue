    ...
    <div class="row col-12">
      ...
      <div class="container text-center mb-4">
        <div class="row row-cols-auto">
          ...
          >
          <div class="col">
            <a class="text-dark" href="#">Establecimientos</a>
          </div>
        </div>
      </div>
      ...
      <div class="row justify-content-between">
        ...
        <div class="col-2">
          ...
          >
            ...
            <option value="nombreEstablecimiento">Nombre del establecimiento</option>
            <option value="estatus">Estado</option>
          </select>
        </div>
        <div class="col-2">
          <input
            v-model="mostrarTodosLosEstablecimientos"
            type="checkbox"
        ...
        <v-data-table density="compact" :items="resultadoFiltrado" :headers="headers">
          <template v-slot:item.actions="{ item }">
            ...
            <v-icon
              ...
              @click="
                ($event) =>
                  confirmDelete(item.nombreEstablecimiento, item.codigoEstablecimiento)
              "
            >
...
const headers = [
  { title: "#", key: "indice" },
  { title: "Nombre", key: "nombreEstablecimiento" },
  { title: "Estado", key: "estatus" },
  { title: "Acción", key: "actions", sortable: false },
];

//GET establecimientos
//#region GET establecimientos
const establecimientos = ref([]);
const establecimientosActivos = ref([]);
const establecimientosSeleccionados = ref([]);
const getEstablecimientos = async () => {
  try {
    ...
    // Filtra los establecimientos cuyo estatus es igual a "A", ignorando mayúsculas/minúsculas
    establecimientosActivos.value = establecimientos.value.filter(
      (establecimiento) => establecimiento.estatus.trim().toUpperCase() === "A"
    );

    if (mostrarTodosLosEstablecimientos.value) {
      establecimientosSeleccionados.value = establecimientos.value;
    } else {
      establecimientosSeleccionados.value = establecimientosActivos.value;
    }

    mapeoDeDato(establecimientosSeleccionados.value);
  } catch (error) {
    Swal.fire({
      title: "Error",
...
const mapeoDeDato = (datos) => {
  resultadoFiltrado.value = datos.map((establecimiento, index) => ({
    indice: index + 1,
...
const tipoBusqueda = ref("Buscar establecimiento");
const filtro = ref("");
const resultadoFiltrado = ref([]);
const tipoFiltro = ref("nombreEstablecimiento");
const mostrarTodosLosEstablecimientos = ref(false);
...
const buscarEstablecimiento = () => {
  ...
  if (textoBusqueda !== "") {
    datosFiltrados = establecimientosSeleccionados.value.filter((establecimiento) => {
      const campoFiltro = establecimiento[tipoFiltro.value]?.toLowerCase(); // Utiliza la propiedad basada en tipoFiltro
...
watch(mostrarTodosLosEstablecimientos, (newValue) => {
  if (!newValue) {
...
const codigoEstablecimientoActualizar = ref("");
const close = ref([]);
...
const confirmDelete = (nombre, codigo) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      deleteEstablecimiento(nombre, codigo);
    }
  });
};

// Método de eliminación (este es el que ya tienes implementado)
const deleteEstablecimiento = async (nombreEstablecimiento, codigoEstablecimiento) => {
  try {
    const response = await axios.delete(
      `/api/establecimiento/delete/${codigoEstablecimiento}`
    );
    Swal.fire({
      title: "Eliminado!",
      text: `El establecimiento ${nombreEstablecimiento} ha sido eliminado.`,
      icon: "success",
    });
    getEstablecimientos();
  } catch (error) {
    Swal.fire({
      title: "Error",
      text: `Hubo un error al intentar eliminar el establecimiento ${nombreEstablecimiento}.`,
      icon: "error",
...
const openModal = (op, codigoEstablecimiento, nombreEstablecimiento, estatus) => {
  clear();
  setTimeout(() => nextTick(() => nameInput.value.focus()), 600);
  operation.value = op;
  codigoEstablecimientoActualizar.value = codigoEstablecimiento;
  if (op == 1) {
...
const save = async () => {
  ...
  if (operation.value == 1) {
    ...
    res = await sendRequest(
      "PUT",
      form.value,
      `/api/establecimiento/update/${codigoEstablecimientoActualizar.value}`,
      ""
    );
...
const descargarPDF = async (datos) => {
  ...
  doc.setFontSize(12);
  doc.text("LISTADO DE ESTABLECIMIENTOS", 71, 7);
  ...
  // Datos de la tabla
  const data = datos.map((establecimiento, index) => [
    index + 1,
  ...
  doc.save("ReporteEstablecimientos.pdf");
};
//#endregion
...
const exportarExcel = () => {
  ...
  const data = datos.map((establecimiento, index) => ({
    ...
    Estado: establecimiento.estatus,
  }));
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Lista de establecimientos");
  ...
  link.href = url;
  link.download = "reporteestablecimientos.xlsx";
